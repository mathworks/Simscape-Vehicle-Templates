function sm_car_optim_traj_vx_regen(mdl, trackname, maxIter)
...
traj_coeff = setup_optim_traj_vx_regen(mdl,trackname);
assignin('base','traj_coeff',traj_coeff);

% Store initial value of max and min speed, coefficient, regen bounds
x0 = [traj_coeff.vmin traj_coeff.vmax traj_coeff.diff_exp 6];
...
LB = [4 5 1 5.1];  % Condition 1
UB = [15 50 2 15];   % Condition 2
% Constraints are defined in the form of Ax <= b
Am=[1 -1 0 0];
bm=[0.5];

[x,~,~,~] = ...
    patternsearch(@(x)obj_traj_vx_regen(x, mdl,trackname,h1_sm_car_optim,h2_sm_car_optim),...
