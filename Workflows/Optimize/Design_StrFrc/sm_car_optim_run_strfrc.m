function [xFinal,fval,CarMetricsFinal] = sm_car_optim_run_strfrc(mdl,Vehicle,par_list,metricName,tgtValue,metricWeights,tTrans)
...
CarMetricsStart = sm_car_perf_metrics_strfrc(out.logsout_sm_car,out.simlog_sm_car,Vehicle,tTrans);
...
[x,fval,~,~] = ...
    patternsearch(@(x)obj_car_optim_strfrc(x, mdl,par_list,Vehicle,metricName,tgtValue,metricWeights,simResStr,fig_h),...
    x0,[],[],[],[],LB,UB,[],options);
...
CarMetricsFinal = sm_car_perf_metrics_strfrc(out.logsout_sm_car,out.simlog_sm_car,Vehicle,0.5);
