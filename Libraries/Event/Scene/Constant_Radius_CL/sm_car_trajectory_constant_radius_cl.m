...
v_lc        = 7;     % Speed start of circle (m/s)
v_lc_end    = 20;    % Speed at end of test (m/s)
...
x_waypoints =  [x_traj_start x_start x_start+x_testv repmat(circle_x,1,num_laps)                        0    1    2    3:1:(x_finish-1)    x_finish];
y_waypoints =  [0            0       0              -repmat(circle_y,1,num_laps)                        0    0    0    zeros(1,47)         0];
vx_waypoints = [1            1       v_lc            linspace(v_lc,v_lc_end,size(circle_x,2)*num_laps)  v_lc_end v_lc_end v_lc_end linspace(v_lc_end,1,47)           1];
...
fig_handle_name = 'h1_sm_car_constant_radius_cl';
...
title('Constant Radius Closed-Loop Trajectory');
...
save Constant_Radius_CL_trajectory_default x y z vx aYaw xTrajectory 
